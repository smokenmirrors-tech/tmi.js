...
var api = function api(options, callback) {
    ...
    // Inside ReactNative, so we can use fetch
    else if (_.isReactNative()) {
        fetch(_.merge(options, { url: url, method: "GET", json: true })).then(function (response) {
            if (response.ok) {
              return response.json();
            }
            callback(response.statis, null, null);
        }).then(function (json) {
